...
export default function CrmContactDetail() {
  ...
  const pocOptions = Array.from(new Set((contact?.visits ?? []).map(v => v.acePoc).filter((p): p is string => !!p))).sort();
        ...
        <div className="lg:col-span-2">
          <Card>
              ...
              {contact.visits.length > 0 && (
                <div className="flex flex-wrap items-end gap-3 pt-3">
                  ...
                  <div>
                    ...
                    <Input id="visit-date-from-contact" type="date" value={visitDateFrom} onChange={e => setVisitDateFrom(e.target.value)} className="h-8 text-sm w-36" data-testid="input-visit-date-from" />
                  </div>
                  <div>
                    <Label htmlFor="visit-date-to-contact" className="text-xs text-muted-foreground mb-1 block">To</Label>
                    <Input id="visit-date-to-contact" type="date" value={visitDateTo} onChange={e => setVisitDateTo(e.target.value)} className="h-8 text-sm w-36" data-testid="input-visit-date-to" />
                  </div>
                  {pocOptions.length > 0 && (
                    <div>
            ...
            <CardContent className="p-0">
              {contact.visits.length === 0 ? (
                <div className="flex items-center justify-center py-12 text-muted-foreground text-sm">No visits recorded yet</div>
              ) : filteredVisits.length === 0 ? (
                <div className="flex items-center justify-center py-12 text-muted-foreground text-sm">No visits match the current filters</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {filteredVisits.map(visit => (
                      <TableRow key={visit.id} data-testid={`row-visit-${visit.id}`}>
                        ...
                        <TableCell className="text-sm text-muted-foreground">{new Date(visit.visitedAt).toLocaleString()}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
