...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/session", async (req, res) => {
    ...
    if (ssoBase) {
      ...
      const callbackUrl = `${appUrl}/api/auth/callback?next=${encodeURIComponent("/")}`;
      return res.json({
        authenticated: false,
  ...
  // ACE Hub → GuestFlow: send a test of the check-in notification email.
  // Secured by the shared platform cron secret (not user auth).
  app.post("/api/internal/cron/test-email", async (req, res) => {
    try {
      const secret = process.env.PLATFORM_CRON_SECRET?.trim();
      const provided = String(req.header("X-Platform-Cron-Secret") || "").trim();
      ...
      if (jobName !== "checkin_notification") {
        return res.status(400).json({ error: `Unsupported job_name: ${jobName}` });
      }
      ...
      res.json({ success: true, message: `Test check-in notification sent to ${testEmail}` });
    } catch (error: any) {
      console.error("[internal/cron/test-email] failed:", error);
