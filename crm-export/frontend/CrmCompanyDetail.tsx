...
function buildRows(company: CompanyDetail) {
  ...
  for (const contact of company.contacts) {
    if (contact.visits.length === 0) {
      rows.push({
        "Title": contact.title ?? "", "First Name": contact.firstName, "Last Name": contact.lastName,
        "Email": contact.email, "Phone": contact.phone ?? "", "Company": company.name,
        "Ace POC": contact.acePoc ?? "", "Event Name": "", "Event Date": "", "Event Location": "", "Visit Date": "",
      });
    } else {
      for (const visit of contact.visits) {
        rows.push({
          "Title": contact.title ?? "", "First Name": contact.firstName, "Last Name": contact.lastName,
          "Email": contact.email, "Phone": contact.phone ?? "", "Company": company.name,
          "Ace POC": visit.acePoc ?? "", "Event Name": visit.eventName ?? "",
          "Event Date": visit.eventDate ?? "", "Event Location": visit.eventLocation ?? "",
...
export default function CrmCompanyDetail() {
  ...
  function clearVisitFilters() {
    setVisitSearch(""); setVisitDateFrom(""); setVisitDateTo(""); setVisitPoc("");
  }
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          <Card>
            ...
            <CardContent className="p-0">
              {company.contacts.length === 0 ? (
                <div className="flex items-center justify-center py-10 text-muted-foreground text-sm">No contacts yet</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {company.contacts.map(contact => (
                      <TableRow key={contact.id} data-testid={`row-contact-${contact.id}`}>
                        ...
                        <TableCell className="text-center"><Badge variant="outline">{contact.visits.length}</Badge></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          <Card>
                ...
                <div className="flex flex-wrap items-end gap-3 pt-3">
                  <div className="flex-1 min-w-[180px]">
                    ...
                    <div className="relative">
                      ...
                      <Input id="visit-search-company" placeholder="Event name or contact…" value={visitSearch} onChange={e => setVisitSearch(e.target.value)} className="pl-8 h-8 text-sm" data-testid="input-visit-search" />
                    </div>
                  </div>
                  <div>
                    <Label htmlFor="visit-date-from-company" className="text-xs text-muted-foreground mb-1 block">From</Label>
                    <Input id="visit-date-from-company" type="date" value={visitDateFrom} onChange={e => setVisitDateFrom(e.target.value)} className="h-8 text-sm w-36" data-testid="input-visit-date-from" />
                  </div>
                  <div>
                    <Label htmlFor="visit-date-to-company" className="text-xs text-muted-foreground mb-1 block">To</Label>
                    <Input id="visit-date-to-company" type="date" value={visitDateTo} onChange={e => setVisitDateTo(e.target.value)} className="h-8 text-sm w-36" data-testid="input-visit-date-to" />
                  </div>
                  {pocOptions.length > 0 && (
                    <div>
                      ...
                      <Select value={visitPoc || "__all__"} onValueChange={v => setVisitPoc(v === "__all__" ? "" : v)}>
                        ...
                        <SelectContent>
                          ...
                          {pocOptions.map(poc => <SelectItem key={poc} value={poc}>{poc}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                  )}
            ...
            <CardContent className="p-0">
              {allVisits.length === 0 ? (
                <div className="flex items-center justify-center py-10 text-muted-foreground text-sm">No visits recorded</div>
              ) : filteredVisits.length === 0 ? (
                <div className="flex items-center justify-center py-10 text-muted-foreground text-sm">No visits match the current filters</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {filteredVisits.map(visit => (
                      <TableRow key={visit.id} data-testid={`row-visit-${visit.id}`}>
                        <TableCell><Link href={`/crm/contacts/${visit.contactId}`} className="text-sm hover:underline">{visit.contactName}</Link></TableCell>
                        ...
                        <TableCell className="text-sm text-muted-foreground">{visit.eventDate ?? new Date(visit.visitedAt).toLocaleDateString()}</TableCell>
