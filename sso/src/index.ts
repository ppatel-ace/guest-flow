...
import { buildJwtPayload, type JwtPayload } from "./azureGroups";
import {
  verifyMicrosoftIdToken,
  extractEmailFromClaims,
  extractNameFromClaims,
  extractGroupsFromClaims,
} from "./microsoftIdToken";
import { ensureRegistryColumns, syncRegistryFromAzureGroups } from "./registrySync";
...
// Optional — only if Azure cannot use Web + public client flows (PKCE-only default)
...
async function initDb() {
  ...
  await ensureRegistryColumns(sql);
}
...
function isTrustedRedirectUri(uri: string): boolean {
  ...
  const extra = (process.env.TRUSTED_REDIRECT_ORIGINS || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  try {
    ...
    if (extra.some((o) => uri.startsWith(o) || origin === o)) return true;

    if (hostname === "localhost" || hostname === "127.0.0.1") return true;

    // Private / LAN IPs — required for IP:port testing before NPM domains exist
    if (process.env.ALLOW_IP_REDIRECTS === "true") {
      if (
        /^10\./.test(hostname) ||
...
app.get("/auth/microsoft/callback", async (req: Request, res: Response) => {
  ...
  // Exchange auth code for tokens (PKCE — client_secret optional for public/SPA clients).
  let idToken: string;
  try {
  ...
  let claims;
  try {
    claims = await verifyMicrosoftIdToken(idToken, AZURE_TENANT_ID, AZURE_CLIENT_ID);
  } catch (err) {
  ...
  try {
    ...
    name = extractNameFromClaims(claims, email);
    groups = extractGroupsFromClaims(claims);
  } catch (err) {
    console.error("[microsoft] claim extraction failed:", err);
  ...
  try {
    ...
    const { employeeId } = await syncRegistryFromAzureGroups(sql, {
      ssoUserId: user.id,
      email: user.email,
      name: user.name,
      groups,
    ...
    console.log(`[microsoft] signed in: ${email} groups=${groups.join(",")} apps=${jwtPayload.apps?.join(",")}`);
